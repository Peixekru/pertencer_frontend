<template>
    <v-container fluid v-if="appStore.appData.firstAccess == 0" class="fill-height"
        :class="appStore.isDarkMode ? 'login-gradiente-dark' : 'login-gradiente-light'">

        <v-container fluid class="pa-0 bg-img-graf" :class="appStore.isDarkMode ? 'bg-img-graf-opacity-dark' : ''" />
    ...
    <v-container v-else fluid class="fill-height pa-0"
        ...
        appStore.appData.access.color == '0' ? 'video-overlay-clean' : 'video-overlay-clean grayscale-filter'">
                <video id="bg-video" autoplay loop muted poster="../assets/img/intro-video-cover.png">
            ...
            <!--Welcome Msg-->
            <WelcomeStartMsg v-if="appStore.appData.firstAccess == 1" class="animate__animated animate__fadeInUp" />

            <!--Welcome Video-->
            <WelcomeStartVideo v-if="appStore.appData.firstAccess >= 2" />
